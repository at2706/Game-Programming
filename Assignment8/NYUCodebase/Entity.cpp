...
GLvoid Entity::fixedUpdate(GameEngine *g){
	...
	if (!isStatic){
		...
		if (y > ASPECT_RATIO_Y - 0.25f && !enableBounce){
			y = ASPECT_RATIO_Y - 0.25f;
			velocity_y = 0.0f;
			return;
		}
		else if (y < -ASPECT_RATIO_Y + 0.25f && !enableBounce){
			y = -ASPECT_RATIO_Y + 0.25f;
			velocity_y = 0.0f;
			return;
		}

		if (y > ASPECT_RATIO_Y - 0.05f){
			y = ASPECT_RATIO_Y - 0.05f;
			velocity_y = -velocity_y;
			setFacing(2.0f * (float)rand() / (float)RAND_MAX - 1);
		}
		else if (y < -ASPECT_RATIO_Y + 0.05f){
			y = -ASPECT_RATIO_Y + 0.05f;
			velocity_y = -velocity_y;
			setFacing(2.0f * (float)rand() / (float)RAND_MAX - 1);
		}
...
GLvoid Entity::setVelocity(GLfloat vx, GLfloat vy){
	GLfloat radian = (facing * PI) / 180.0f;
	velocity_x = vx * cos(radian);
	velocity_y = vy * sin(radian);
}
...
GLboolean Entity::collidesWith(Entity *e){
	...
	GLfloat top = y + ((sprite->height) / 2);
	GLfloat bot = y - ((sprite->height) / 2);
	GLfloat left = x - ((sprite->width) / 2);
	GLfloat right = x + ((sprite->width) / 2);

	GLfloat etop = e->y + ((e->sprite->height) / 2);
	GLfloat ebot = e->y - ((e->sprite->height) / 2);
	GLfloat eleft = e->x - ((e->sprite->width) / 2);
	GLfloat eright = e->x + ((e->sprite->width) / 2);
...
GLboolean Entity::collisionCheck(Entity *e){
	...
	GLfloat radian = (facing * PI) / 180.0f;
	if (enableBounce){
		x -= (penetrations[0].x - 0.0001f);
		y -= (penetrations[0].y - 0.0001f);
...
GLvoid Entity::collisionAxis(GameEngine *g){
	vector<Entity*>::iterator end = entities.end();
	for (vector<Entity*>::iterator it2 = entities.begin(); it2 != end; ++it2){
