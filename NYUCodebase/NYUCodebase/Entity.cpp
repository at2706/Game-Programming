...
Entity::Entity(const char *image_path, GLfloat posX, GLfloat posY, GLfloat width, GLfloat height, GLfloat rotate, GLfloat s)
	: speed(s), rotation(rotate){
	if (image_path != "") loadTexture(image_path);
	setPos(posX, posY);
...
GLboolean Entity::collision(Entity& e){
	...
	GLfloat etop = e.y + (e.height / 2);
	GLfloat ebot = e.y - (e.height / 2);
	GLfloat eleft = e.x - (e.width / 2);
	GLfloat eright = e.x + (e.width / 2);
